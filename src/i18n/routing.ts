import {defineRouting} from 'next-intl/routing';
import {createNavigation} from 'next-intl/navigation';

export const routing = defineRouting({
  locales: ['mn'],
  defaultLocale: 'mn',
  localeDetection: false,
  localePrefix: 'as-needed',
  pathnames: {
    '/': '/',
    '/about': '/about',
    '/contact': '/contact',
    '/essentials': '/essentials',
    '/essentials/[id]': '/essentials/[id]',
    '/city/[slug]': '/city/[slug]',
    '/city/[slug]/place/[placeSlug]': '/city/[slug]/place/[placeSlug]',
    '/tours/custom': '/tours/custom',
    '/guides/apply': '/guides/apply',
    '/experiences': '/experiences',
    '/experiences/[id]': '/experiences/[id]',
    '/flights': '/flights',
    '/guides': '/guides',
    '/guides/[slug]': '/guides/[slug]',
    '/blog': '/blog',
    '/blog/[slug]': '/blog/[slug]',
    '/learn': '/learn',
    '/planner': '/planner',
    '/planner/shared/[id]': '/planner/shared/[id]',
    '/privacy': '/privacy',
    '/login': '/login',
    '/signup': '/signup',
    '/account': '/account',
    '/account/messages': '/account/messages',
    '/account/saved': '/account/saved',
    '/transfer': '/transfer',
    '/transfer/history': '/transfer/history',
    '/transfer/confirmation/[id]': '/transfer/confirmation/[id]',
    '/transfer/trip/[id]': '/transfer/trip/[id]',
    '/driver': '/driver',
    '/driver/apply': '/driver/apply',
    '/driver/history': '/driver/history',
    '/admin': '/admin',
    '/admin/cities': '/admin/cities',
    '/admin/cities/new': '/admin/cities/new',
    '/admin/cities/[id]/edit': '/admin/cities/[id]/edit',
    '/admin/blog': '/admin/blog',
    '/admin/blog/new': '/admin/blog/new',
    '/admin/blog/[id]/edit': '/admin/blog/[id]/edit',
    '/admin/tours': '/admin/tours',
    '/admin/places': '/admin/places',
    '/admin/places/new': '/admin/places/new',
    '/admin/places/[id]/edit': '/admin/places/[id]/edit',
    '/admin/places/import': '/admin/places/import',
    '/admin/guides': '/admin/guides',
    '/admin/guides/new': '/admin/guides/new',
    '/admin/guides/[id]/edit': '/admin/guides/[id]/edit',
    '/admin/learn': '/admin/learn',
    '/admin/learn/new': '/admin/learn/new',
    '/admin/learn/[id]/edit': '/admin/learn/[id]/edit',
    '/admin/transfers': '/admin/transfers',
    '/admin/flights': '/admin/flights',
    '/admin/flights/new': '/admin/flights/new',
    '/admin/flights/[id]/edit': '/admin/flights/[id]/edit',
    '/admin/drivers': '/admin/drivers',
    '/admin/users': '/admin/users',
  }
});

export const {Link, redirect, usePathname, useRouter, getPathname} =
  createNavigation(routing);
