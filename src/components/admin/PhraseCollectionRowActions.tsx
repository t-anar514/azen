...
interface PhraseCollectionRowActionsProps {
  id: string
  published: boolean
}

export function PhraseCollectionRowActions({ id, published }: PhraseCollectionRowActionsProps) {
  const router = useRouter()
  const [busy, setBusy] = React.useState(false)

  async function togglePublished() {
    setBusy(true)
    await fetch(`/api/admin/learn/${id}`, {
      method: "PATCH",
  ...
  return (
    <div className="flex items-center gap-2">
      <Button asChild variant="outline" size="sm">
        <Link href={`/admin/learn/${id}/edit`}>Edit</Link>
      </Button>
